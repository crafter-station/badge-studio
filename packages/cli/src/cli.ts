...
const nextSteps = ["badge-studio styles list", "badge-studio schema"];
...
async function run() {
	...
	if (command === "styles list") {
		...
		emit(
			{ styles },
			["badge-studio design create --style opalo-lunar --out badge.json"],
	...
	if (command === "schema") {
		emit(
			...
			["badge-studio design validate --file badge.json"],
	...
	if (command === "design create") {
		if (!values.style)
			throw new InputError("Supply --style. Run badge-studio styles list for IDs.");
		...
		emit(
			...
			path
				? [
						`badge-studio design validate --file ${JSON.stringify(path)}`,
